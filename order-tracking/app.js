(function () {
  ...
  var SYSTEM_EMAIL = 'system@maitool.internal';
  ...
  function renderCustomerLogin() {
    shell(
      '<div class="login-view"><div class="login-card">' +
        ...
        '<button class="back-link" id="toSystemGate">System Admin: Sales Accounts &rarr;</button>' +
      '</div></div>',
      { subtitle: 'Customer Portal' }
    );
    document.getElementById('toAdminLogin').addEventListener('click', renderAdminLogin);
    document.getElementById('toSystemGate').addEventListener('click', renderSystemGate);
  ...
  function renderAdminPanel() {
    shell(
      '<div class="main">' +
        ...
        '<p class="page-desc">Maintain order production status, warranty expiration dates, and customer accounts. <span style="color:var(--ink-700);">The data below only shows records associated with you.</span></p>' +
        '<div class="tab-row">' +
        ...
        '<div class="tab-panel" id="tabCategories">' +
          '<div class="panel-toolbar"><span class="hint">Shared across all sales reps</span>' +
        ...
        '</div>' +
        '<p class="footnote">Orders and customer accounts are scoped to the sales rep who created them. Categories are shared company-wide.</p>' +
      '</div>' + modalHtml(),
    ...
    document.querySelectorAll('.tab-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        var map = { orders: 'tabOrders', customers: 'tabCustomers', categories: 'tabCategories' };
  ...
  function loadAdminData() {
    Promise.all([
      ...
      ['adminOrdersBody', 'adminCustsBody', 'adminCatsBody'].forEach(function (id) {
  ...
  function openCustForm(customerId) {
    ...
    setModal(
      ...
      '<div class="modal-grid">' +
        ...
        '<div class="field full"><div class="field-note">This customer is automatically assigned to you. Other sales accounts will not see this customer or their orders.</div></div>' +
      '</div>' +
  ...
  // ==============================================================
  // SYSTEM: gate + Sales Account Management
  // ==============================================================
  function renderSystemGate() {
    shell(
      '<div class="login-view"><div class="login-card">' +
        '<h1 class="login-title">Sales Account Management</h1>' +
        '<p class="login-desc">This section is for adding, editing, or removing sales team login accounts. It requires the system admin password.</p>' +
        '<div class="login-error" id="sysErr">Incorrect password.</div>' +
        '<div class="field"><label>System Admin Password</label><input type="password" id="sysPw" autocomplete="off"></div>' +
        '<button class="login-btn" id="sysBtn">Enter</button>' +
        '<button class="back-link" id="sysBack">&larr; Back to Home</button>' +
      '</div></div>',
      { subtitle: 'System Admin' }
    );
    document.getElementById('sysBack').addEventListener('click', renderCustomerLogin);
    function doLogin() {
      var pw = document.getElementById('sysPw').value;
      var errEl = document.getElementById('sysErr');
      errEl.classList.remove('show');
      supabase.auth.signInWithPassword({ email: SYSTEM_EMAIL, password: pw }).then(function (res) {
        if (res.error) { errEl.classList.add('show'); return; }
        renderAcctManage();
      });
    }
    document.getElementById('sysBtn').addEventListener('click', doLogin);
    document.getElementById('sysPw').addEventListener('keydown', function (e) { if (e.key === 'Enter') doLogin(); });
  }
  ...
  function renderAcctManage() {
    shell(
      '<div class="main">' +
        '<h1 class="page-title">Sales Account Management</h1>' +
        '<p class="page-desc">Maintain sales team login accounts, separate from order and customer account management.</p>' +
        '<div class="panel-toolbar"><span class="hint"><span id="acctCountHint">0</span> sales accounts</span>' +
        '<button class="primary-btn" id="addAcctBtn">+ New Sales Account</button></div>' +
        '<div class="table-wrap"><table><thead><tr><th>Name</th><th>Login ID</th><th></th></tr></thead><tbody id="acctsBody"></tbody></table></div>' +
        '<p class="footnote">This screen requires the system admin password to reach, separate from any individual sales login.</p>' +
      '</div>' + modalHtml(),
      { userLabel: 'System Admin', subtitle: 'System Admin' }
    );
    bindModalOverlay();
  ...
  function loadAccounts() {
    supabase.from('order_admins').select('login_id, name').order('name').then(function (res) {
      if (res.error) throw new Error(res.error.message);
      salesAccounts = res.data;
      renderAcctsTable();
    }).catch(function (e) {
      document.getElementById('acctsBody').innerHTML = '<tr><td colspan="3" style="color:#c23b23; text-align:center; padding:26px;">' + esc(e.message) + '</td></tr>';
    });
  }
  ...
  function renderAcctsTable() {
    ...
    document.getElementById('acctsBody').innerHTML = salesAccounts.map(function (a) {
      return '<tr><td class="product-name">' + esc(a.name) + '</td><td class="order-id">' + esc(a.login_id) + '</td>' +
        ...
        '<button class="icon-btn danger" data-confirm="0" data-del="' + esc(a.login_id) + '"' + (disableDelete ? ' disabled title="At least one sales account must remain"' : '') + '>Delete</button></div></td></tr>';
    }).join('');
    ...
    document.querySelectorAll('#acctsBody [data-del]').forEach(function (b) {
      if (b.disabled) return;
      b.addEventListener('click', function () {
        confirmThenDelete(b, function () { return invokeFn({ action: 'delete_admin', login_id: b.dataset.del }).then(loadAccounts); });
      });
    });
  }
  ...
  function openAcctForm(loginId) {
    ...
    document.getElementById('mSave').addEventListener('click', function () {
      ...
      chain.then(function () { closeModal(); loadAccounts(); }).catch(function (e) { alert(e.message); });
    });
  }
  ...
  function boot() {
    supabase.auth.getSession().then(function (res) {
      ...
      if (user.email === SYSTEM_EMAIL) { renderAcctManage(); return; }
