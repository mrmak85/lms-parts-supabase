(function () {
  ...
  function copyrightHtml() {
    return '<br><span style="opacity:.7;">&copy; ' + new Date().getFullYear() + ' LMS/FRAMEMAC &middot; Built by Wilson Mai</span>';
  }
      ...
      '<div class="app">' +
        '<div class="topbar">' +
          '<div class="brand">' +
            ...
            '<div><div class="brand-text">LMS/FRAMEMAC Spare Parts</div><div class="brand-sub">' + (opts.subtitle || '') + '</div></div>' +
          '</div>' +
  ...
  function renderCatalog() {
    shell(
      '<div class="catalog-layout">' +
        ...
        selectionPanelHtml() +
      '</div>' +
      '<p class="footnote">LMS/FRAMEMAC Machinery &middot; Contact: Wilson Mai (wilson@lmsmachinery.com)' + copyrightHtml() + '</p>',
    ...
    document.getElementById('generatePdfBtn').addEventListener('click', function () { generateInvoicePdf(false); });
    document.getElementById('emailPdfBtn').addEventListener('click', function () { generateInvoicePdf(true); });
    ...
    return (
      '<aside class="selection-panel">' +
        ...
        '<div style="margin-top:16px; padding-top:14px; border-top:1px solid var(--steel-800);">' +
          ...
          '<div class="cart-note">Excl. shipping &amp; tariffs, confirmed by LMS/FRAMEMAC after receiving your list.</div>' +
        '</div>' +
        ...
        '<div class="field-note" style="margin-top:8px;">The email button opens your email app with everything filled in — you\'ll just need to attach the PDF that downloads (browsers don\'t allow websites to attach files automatically).</div>' +
      '</aside>'
    );
  }
  ...
  function generateInvoicePdf(alsoEmail) {
    var ids = Object.keys(cart).filter(function (id) { return cart[id] > 0; });
    ...
    supabase.rpc('next_invoice_number').then(function (res) {
      ...
      if (alsoEmail) openEmailClient(invoiceNumber);
    });
  }

  function openEmailClient(invoiceNumber) {
    var company = document.getElementById('custCompany').value.trim() || 'your company';
    var subject = 'Spare Parts Selection — ' + company + ' — ' + invoiceNumber;
    var body =
      'Hi Wilson,\n\n' +
      'Please find our spare parts selection attached (reference ' + invoiceNumber + ').\n\n' +
      '(A PDF just downloaded to this device — please attach that file before sending, since a web page cannot attach it automatically.)\n\n' +
      'Thanks,\n' + company;
    // Small delay so the browser's file-download dialog/action isn't
    // interrupted by immediately navigating to a mailto: link.
    setTimeout(function () {
      window.location.href = 'mailto:wilson@lmsmachinery.com?subject=' + encodeURIComponent(subject) + '&body=' + encodeURIComponent(body);
    }, 400);
  }
  ...
  function buildPdf(ids, invoiceNumber, jsPDFCtor) {
    ...
    doc.text('LMS/FRAMEMAC Spare Parts — Selection List', margin, y); y += 18;
    ...
    var disclaimer = 'Prices shown are estimates, excl. shipping & tariffs. Shipping is added by LMS/FRAMEMAC after this list is received. Tariffs are the buyer\'s responsibility. Final pricing confirmed by LMS/FRAMEMAC before order confirmation.';
    ...
    doc.text('Please send this PDF to your LMS/FRAMEMAC contact: Wilson Mai (wilson@lmsmachinery.com)', margin, y);

    var filenameBase = (company !== '-' ? company : 'LMS-FRAMEMAC-Selection').replace(/[^a-z0-9]+/gi, '-');
