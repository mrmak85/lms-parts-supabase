(function () {
  ...
  function renderCatalog() {
    shell(
      '<div class="main">' +
        ...
        '<p class="page-desc">Tick the parts you need, set quantities, then click <b>Selection</b> to review and download a PDF you can send to your LMS contact. Prices are <b>excl. shipping &amp; tariffs</b>.</p>' +
        '<div class="filters">' +
      ...
      cartDrawerHtml(),
      {
        ...
        topbarActions: '<button class="cart-btn" id="cartBtn">Selection <span class="cart-count" id="cartCount">0</span></button>'
      }
    );
    ...
    document.getElementById('drawerOverlay').addEventListener('click', function (e) { if (e.target.id === 'drawerOverlay') closeDrawer(); });
  ...
  function updateCartCount() {
    ...
    if (!el) return;
    el.textContent = Object.keys(cart).reduce(function (s, id) { return s + cart[id]; }, 0);
  }
  ...
  function cartDrawerHtml() {
    return (
      '<div class="drawer-overlay" id="drawerOverlay"><div class="drawer">' +
        '<div class="drawer-head"><h2 class="drawer-title">Your Selection</h2><button class="drawer-close" id="drawerCloseBtn">Close</button></div>' +
        '<div class="drawer-body">' +
          '<div id="cartLines"></div>' +
          '<div style="margin-top:20px; padding-top:16px; border-top:1px solid var(--steel-800);">' +
          ...
          '<div class="field"><label>Contact Name</label><input type="text" id="custName"></div>' +
          '<div class="field"><label>Email</label><input type="text" id="custEmail"></div>' +
          '<div class="field"><label>Phone (optional)</label><input type="text" id="custPhone"></div>' +
          '<div class="field"><label>Notes (optional)</label><textarea id="custNotes" rows="3"></textarea></div>' +
        '</div>' +
        '<div class="drawer-foot"><button class="generate-btn" id="generatePdfBtn">Download Selection PDF</button></div>' +
      '</div></div>'
    );
  }

  function openDrawer() { renderCartDrawer(); document.getElementById('drawerOverlay').classList.add('open'); }
  ...
  function renderCartDrawer() {
    ...
    if (ids.length === 0) {
      linesEl.innerHTML = '<div class="empty-state">No parts selected yet.</div>';
    ...
    linesEl.querySelectorAll('[data-cart-remove]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        updateCartCount(); renderCartDrawer(); renderGrid();
      });
    });
  }
  ...
  function buildPdf(ids, invoiceNumber, jsPDFCtor) {
    ...
    ['Company: ' + company, 'Contact: ' + name, 'Email: ' + email, 'Phone: ' + phone].forEach(function (l) { doc.text(l, margin, y); y += 13; });
    if (notes) {
      y += 4; doc.setFont('helvetica','bold'); doc.text('Notes:', margin, y); y += 13;
      doc.setFont('helvetica','normal');
      var noteLines = doc.splitTextToSize(notes, pageWidth - margin*2);
      doc.text(noteLines, margin, y); y += noteLines.length * 12;
    }
    y += 14;
